...
//        System.out.println(validIPAddress("1.1.1.1."));
        System.out.println(validIPAddress("2001:0db8::0000:0:8A2E:0370:733a"
        ));
    }
    public static String validIPAddress(String IP) {
        ...
        if (splits.length == 4) {
            for (int i = 0; i < 4; i++) {
                try {
                    int val = Integer.parseInt(splits[i]);
                    if (val < 0 || val > 255) {
                        return "Neither";
                    }
                    if (!splits[i].equals(String.valueOf(val))) {
                        return "Neither";
                    }
                } catch (Exception e) {
                    return "Neither";
        ...
        for (String str : splits) {
            ...
            for (int j = 0; j < Math.min(4, str.length()); j++) {
